...
- Sally 默认带入“金融转 AI 产品”模拟同学档案；李彦宏默认带入独立的“林小北 · 第一份 AI 产品实习”演示 case（大三信息管理学生），已随 `be1ce9f` 部署，线上核对见最新迭代日志。两者继续默认带入独立档案；已随 `857a390` 上线移除聊天／通话档案面板和聊天状态栏，欢迎语不复述用户信息，空白模式仅保留底层接口；对话仅使用本次连接历史，切换专家会关闭连接，返回真实专家时开启新会话。“我的”三页共用大学生示例数据，任务、成果、兴趣反馈与复盘分开存入 localStorage；游戏化测评结果单独保存，不覆盖示例 RIASEC，也尚未自动接入专家咨询上下文。
- 用户最新要求优先于历史方案；历史截图、压缩包和方案不是当前源码的替代来源。
...
| [static/consultation-ui.js](static/consultation-ui.js) | Sally、李彦宏与公共专家页面的 UI 适配、通用欢迎语、输入区异常反馈及真实通话入口 |
...
| [static/robin-li-office.png](static/robin-li-office.png)、[static/demo-planner-xu.png](static/demo-planner-xu.png) | 当前展示中的专家头像；李彦宏使用明亮办公室版本，许望舒使用职业规划师演示头像 |
