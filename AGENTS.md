...
- Sally 默认带入“金融转 AI 产品”模拟同学档案；李彦宏默认带入独立的“林小北 · 第一份 AI 产品实习”演示 case（大三信息管理学生），已随 `be1ce9f` 部署，线上核对见最新迭代日志。两者继续默认带入独立档案；本地已移除聊天／通话档案面板和聊天状态栏，欢迎语不复述用户信息，空白模式仅保留底层接口；对话仅使用本次连接历史，切换专家会关闭连接，返回真实专家时开启新会话。“我的”三页共用大学生示例数据，任务、成果、兴趣反馈与复盘分开存入 localStorage；游戏化测评结果单独保存，不覆盖示例 RIASEC，也尚未自动接入专家咨询上下文。
